...
async function uploadImage(file) {
    ...
    if (file.size > maxSize) {
        alert('Archivo demasiado grande. Tamaño máximo: 5MB.');
        return null;
    }
...
async function createNews(title, content, imageFile, category, tags, isFeatured, priority) {
    try {
        ...
        if (!title || title.trim().length === 0 || title.length > 200) {
            alert('Título inválido. Debe tener entre 1 y 200 caracteres.');
            return;
        }
        if (!content || content.trim().length === 0 || content.length > 10000) {
            alert('Contenido inválido. Debe tener entre 1 y 10,000 caracteres.');
            return;
        }
...
function renderNews() {
    ...
    newsToRender.forEach(news => {
        ...
        if (imageUrl) {
            ...
            img.src = imageUrl;
            img.alt = newsTitle; // Browser handles attribute escaping
            img.className = 'news-image';
            img.onclick = () => showNewsDetail(newsId);
            card.appendChild(img);
        } else {
            const noImage = document.createElement('div');
            noImage.className = 'news-image';
            noImage.style.cssText = 'background: linear-gradient(45deg, #1e293b, #334155); display:flex; align-items:center; justify-content:center; color:#64748b; cursor: pointer;';
            noImage.onclick = () => showNewsDetail(newsId);
            const span = document.createElement('span');
            span.textContent = 'Sin Imagen';
            noImage.appendChild(span);
        ...
        categorySpan.className = `news-category ${categoryClass}`;
        categorySpan.textContent = `${categoryEmojis[newsCategory] || '📢'} ${newsCategory}`;
        ...
        if (STATE.currentUser && STATE.currentUser.role === 'admin') {
            ...
            editBtn.onclick = () => openEditModal(newsId);
